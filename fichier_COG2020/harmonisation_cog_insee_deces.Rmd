...
```{r insee_deces, echo=FALSE}
# Chargement du jeu de données 

insee_deces <- readr::read_csv("insee_deces.csv",col_types = cols(code_lieu_deces = col_character(),
...
```{r codes, echo=FALSE}
print(paste("Le fichier comporte", length(unique(insee_deces$code_lieu_naissance)), "COG uniques de naissance.", sep = " "))
print(paste("Le fichier comporte", length(unique(insee_deces$code_lieu_deces)), "COG uniques de décès.", sep = " "))
```
...
*Les mouvements communaux depuis 1970*
...
evenements <- data.frame("Année"=c(1971,1976,2010,2015),"Évènement"= c("Loi Marcellin","Découpage de la Corse","Loi RCT","Loi NOTRe"))
#evenements$Évènement <- as.factor(evenements$Évènement)
#levels(evenements$Évènement) <- c("Loi Marcellin","Découpage de la Corse","Réforme des collectivités territoriales","Loi NOTRe")

# Plot
f1 <-ggplot2::ggplot() + geom_line(data=mvt70_ag, aes(Année,Fréquence),color="black") + geom_vline(data=evenements,aes(xintercept=Année, color=Évènement),linetype="dashed", size=1)
...
# Plot
f2 <-ggplot2::ggplot(mvt_type_ag, aes(Modification,Fréquence)) +  geom_bar(stat="identity")+
labs( x = "Type de mouvement", y = "Fréquence",title ="Mouvements communaux depuis 1970")+ coord_flip()
...
print(paste("Tous les enregistrements ont-ils été conservés ?", nrow(sel0)+nrow(sel)==nrow(insee_deces), sep = " "))
...
print(paste("Combien de codes ont disparu dans les défusions ?", nrow(defusions[!(defusions$com_av %in% defusions$com_ap),]), sep = " ")) # Aucun cas de perte de code
...
mvtcommune <- mvtcommune[!(mvtcommune$typecom_av %in% c("COMA","COMD"))& !(mvtcommune$typecom_ap %in% c("COMA","COMD")),]
```
...
# L'Oudon: Le code 14624, supprimé en 1973 suite à une fusion, a été réaffecté à un ensemble de communes ayant refusionné après une défusion en 2014 
...
# Les communes supprimées: 6 communes ont été supprimées à la suite de la partition de leur territoire entre plusieurs autres communes. Il n'est pas possible d'attribuer un code commune unique en vigueur à ces communes qui se sont divisées. Ces enregistrements sont considérés comme non localisables. Le code commune "inconnu" est attribué. 

tab2 <- tab1[tab1$mod == "30",]
# suppression de mvt_commune
mvtcommune <- mvtcommune[!((mvtcommune$com_av %in%  tab2$com_av) & (mvtcommune$com_ap %in% tab2$com_ap)) ,]
...
mvtcommune <- mvtcommune[!((mvtcommune$com_av %in%  tab3$com_av) & (mvtcommune$com_ap %in% tab3$com_ap) & (mvtcommune$date_eff %in% tab3$date_eff)) ,]

# mouvements uniques: sélection des codes "après" qui ne réapparaissent pas dans les codes "avant"
mvt_unique <- mvtcommune[!(mvtcommune$com_ap %in% mvtcommune$com_av),]

# mouvements 1: sélection des codes "après" qui réapparaissent dans les codes "avant" (= autre changement)
mvt_1 <- mvtcommune[mvtcommune$com_ap %in% mvtcommune$com_av,]

# mouvements 2: sélection des codes "après" du mouvement 1 qui réapparaissent encore dans les codes "avant" (= autre changement)
mvt_2 <- mvt_1[mvt_1$com_ap %in% mvt_1$com_av,]

# mouvements 3: sélection des codes "après" du mouvement 2 qui réapparaissent encore dans les codes "avant" (= autre changement)
...
### vérification qu'il n'y a pas eu de perte d'enregistrement
print(paste("Tous les enregistrements ont-ils été conservés ?", nrow(sel0)+nrow(sel1)+ nrow(sel2) + nrow(sel3)+ nrow(sel)==nrow(insee_deces), sep = " "))

### Combien d'enregistrements ont été récupérés 
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel3)))
print(paste("Soit", nrow(sel3)/nrow(insee_deces)*100,"% de la base.", sep=" "))
...
sel5 <- sel[sel$code_lieu_naissance %in% c("91352","92352","93352", "94352", "91","92", "20000","123"),]
sel5$code_nai20 <- paste("99999") 
sel <- sel[!(sel$code_lieu_naissance %in% c("91352","92352","93352", "94352", "91","92","20000","123")),]

### vérification qu'il n'y a pas eu de perte d'enregistrement
print(paste("Tous les enregistrements ont-ils été conservés ?", nrow(sel0)+nrow(sel1)+ nrow(sel2) + nrow(sel3)+ nrow(sel4) + nrow(sel5)+ nrow(sel)==nrow(insee_deces), sep = " "))

#### progression ####
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel4)+nrow(sel5)))
print(paste("Soit", (nrow(sel4)+nrow(sel5))/nrow(insee_deces)*100,"% de la base.", sep=" "))
print(paste("Il reste",length(unique(sel$code_lieu_naissance)), "/", length(unique(insee_deces$code_lieu_naissance)), "codes uniques à traiter", sep = " " ))
...
sel <- sel[!(sel$code_lieu_naissance %in% c("97127","97123","97801","97701","97501","97502")),]

#### progression ####
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel6)+nrow(sel7)+nrow(sel8)+nrow(sel9)))
print(paste("Soit", (nrow(sel6)+nrow(sel7)+nrow(sel8)+nrow(sel9))/nrow(insee_deces)*100,"% de la base.", sep=" "))
print(paste("Il reste",length(unique(sel$code_lieu_naissance)), "/", length(unique(insee_deces$code_lieu_naissance)), "codes uniques à traiter", sep = " " ))
...
print(paste("Les codes restés sans correspondance concernent", nrow(sel10),"enregistrements."))
```
...
# verif 
print(paste("Tous les enregistrements ont ils été conservés?", nrow(sel)==nrow(insee_deces), sep=" "))
...
# sel <- sel[!(sel$code_lieu_naissance %in% c("95025","95065","66131","66235","01220","01459","54448","54454")),]
...
# 
# # Paris
# sel16 <- sel[sel$code_lieu_naissance %in% c("75212"),]
# sel16$code_nai20 <- paste("75112")
# sel17 <- sel[sel$code_lieu_naissance %in% c("75213"),]
# sel17$code_nai20 <- paste("75113")
# sel18 <- sel[sel$code_lieu_naissance %in% c("75214"),]
# sel18$code_nai20 <- paste("75114")
# sel19 <- sel[sel$code_lieu_naissance %in% c("75215"),]
# sel19$code_nai20 <- paste("75115")
# 
# # Corbeil-Essonnes
# sel20 <- sel[sel$code_lieu_naissance %in% c("91221"),]
# sel20$code_nai20 <- paste("91174")
# 
# # Nantes
# sel21 <- sel[sel$code_lieu_naissance %in% c("44309"),]
# sel21$code_nai20 <- paste("44109")
# 
# # Retirer les corrections aux cas par cas
# sel <- sel[!(sel$code_lieu_naissance %in% c("75212","75213","75214","75215","13155","91221","44309","97501")),]
# 
# print(paste("Il reste",length(unique(sel$code_lieu_naissance)), "/", length(unique(insee_deces$code_lieu_naissance)), "codes uniques à traiter", sep = " " ))
...
Une fois les corrections manuelles effectuées, reprendre le chunck {r agregation} au point 6 en ajoutant les sel10 à 21 au rbind. 
...
## Création d'une nouvelle variable code_dec20  pour stocker les codes harmonisés
...
### vérification qu'il n'y a pas eu de perte d'enregistrement
print(paste("Tous les enregistrements ont-ils été conservés ?", nrow(sel0)+nrow(sel1)+ nrow(sel2)+nrow(sel3) + nrow(sel4)+ nrow(sel)==nrow(insee_deces), sep = " "))

### Combien d'enregistrements ont été récupérés 
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel4)))
print(paste("Soit", nrow(sel4)/nrow(insee_deces)*100,"% de la base.", sep=" "))
...
# Non prise en compte de l'indépendance de l'Algérie dans l'enregistrement du code 
sel6 <- sel[sel$code_lieu_deces %in% c("91352","92352","93352", "94352", "91","92", "20000","123"),] # Un seul cas pour les lieux de décès
sel6$code_dec20  <- paste("99999") 
sel <- sel[!(sel$code_lieu_deces %in% c("91352","92352","93352", "94352", "91","92","20000","123")),]

### vérification qu'il n'y a pas eu de perte d'enregistrement
print(paste("Tous les enregistrements ont-ils été conservés ?", nrow(sel0)+nrow(sel1)+ nrow(sel2) + nrow(sel3)+ nrow(sel4) + nrow(sel5)+nrow(sel6)+ nrow(sel)==nrow(insee_deces), sep = " "))

#### progression ####
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel5)+nrow(sel6)))
print(paste("Soit", (nrow(sel5)+nrow(sel6))/nrow(insee_deces)*100,"% de la base.", sep=" "))
print(paste("Il reste",length(unique(sel$code_lieu_deces)), "/", length(unique(insee_deces$code_lieu_deces)), "codes uniques à traiter", sep = " " ))
...
sel <- sel[!(sel$code_lieu_deces %in% c("97127","97123","97801","97701","97501","97502")),]

#### progression ####
print(paste("Combien d'enregistrements ont été récupérés ?", nrow(sel7)+nrow(sel8)+nrow(sel9)+nrow(sel10)))
print(paste("Soit", (nrow(sel7)+nrow(sel8)+nrow(sel9)+nrow(sel10))/nrow(insee_deces)*100,"% de la base.", sep=" "))
print(paste("Il reste",length(unique(sel$code_lieu_deces)), "/", length(unique(insee_deces$code_lieu_deces)), "codes uniques à traiter", sep = " " ))

# Identifier les corrections restantes 
a_recoder <- as.data.frame(table(as.factor(sel$code_lieu_deces)))
print(paste("Le nombre moyen d'occurences pour chaque code resté sans correspondance est",mean(a_recoder$Freq)))
print(paste("Le nombre maximal d'occurences pour un code resté sans correspondance est",max(a_recoder$Freq)))

# Coder les codes restants comme inconnus
sel11 <- sel
sel11$code_dec20 <- paste("INCONNU")
print(paste("Les codes restés sans correspondances concernent", nrow(sel11),"enregistrements."))
